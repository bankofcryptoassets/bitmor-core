...
contract UniswapV4Swapper is ISwapAdaptor {
    ...
    /**
     * @notice Helper function to build PoolKey dynamically
     * @param tokenIn Input token address
     * @param tokenOut Output token address
     * @param fee Pool fee tier
     * @param tickSpacing Pool tick spacing
     * @param hooks Pool hooks contract address
     ...
     * @param tickSpacing Pool tick spacing
     * @param hooks Pool hooks contract address (address(0) if no hooks)
    ...
    function getMaxTokenInAmount(
        ...
        (PoolKey memory poolKey, bool zeroForOne) = _buildPoolKey(tokenIn, tokenOut, fee, tickSpacing, hooks);
     ...
     * @dev Uses UniversalRouter for the actual swap execution. Used for closing loans (BTC → USDC)
     * @param tokenIn Input token address
     * @param tokenOut Output token address
     * @param fee Pool fee tier (e.g., 3000 = 0.3%)
     * @param tickSpacing Pool tick spacing
    ...
    function swapExactInput(
        ...
        IERC20(tokenIn).safeTransferFrom(msg.sender, address(universalRouter), exactAmountIn);

        // Execute the swap with exact input
        _executeSwapExactIn(tokenIn, tokenOut, fee, tickSpacing, hooks, exactAmountIn, minAmountOut);
     ...
     * @dev Uses UniversalRouter for the actual swap execution. Used for opening loans (USDC → BTC)
     * @param tokenIn Input token address
     * @param tokenOut Output token address
     * @param fee Pool fee tier (e.g., 3000 = 0.3%)
     * @param tickSpacing Pool tick spacing
    ...
    function swapExactOutput(
        ...
        IERC20(tokenIn).safeTransferFrom(msg.sender, address(universalRouter), maxAmountIn);

        // Execute the swap
        _executeSwapExactOut(tokenIn, tokenOut, fee, tickSpacing, hooks, exactAmountOut, maxAmountIn);
     ...
     *      Uses SETTLE (not SETTLE_ALL) with payerIsUser=false to pay from this contract
     */
    function _executeSwapExactIn(
        ...
        // Using SETTLE (not SETTLE_ALL) with payerIsUser=false to pay from this contract
        ...
        // payerIsUser = false means the router pays from its own balance (we transferred tokens to it)
        ...
        // Take the output currency to this contract
     ...
     *      V4_SWAP uses SETTLE with payerIsUser=false to pay from the router
     *      SWEEP returns unused input tokens to this contract
     */
    function _executeSwapExactOut(
        ...
        // Param 1: SETTLE - (currency, amount, payerIsUser)
        // Use OPEN_DELTA (0) to settle exactly what's owed, payerIsUser=false for router to pay
        ...
        // Execute via UniversalRouter with 1 hour deadline
        universalRouter.execute(commands, inputs, block.timestamp + 3600);
    }
