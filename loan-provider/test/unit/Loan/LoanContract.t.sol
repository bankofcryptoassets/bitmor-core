...
contract LoanContract is BaseLoanTest {
    ...
    /// @notice Constructor reverts with ZeroAddress when swapAdapterWrapper is zero.
    /// @dev This test specifically covers line 83 in Loan.sol (the revert inside Loan constructor)
    function test_loan_constructor_zeroSwapAdapter_reverts() public {
        ConstructorParams memory p = _loadConstructorParams();
        ...
        new Loan(
            ...
            p.debtAsset,
            p.btc,
            address(0), // swapAdapterWrapper = address(0)
            p.zQuoter,
            p.premiumCollector,
            p.preClosureFeeBps,
            p.gracePeriod,
    ...
    /// @notice Constructor reverts with ZeroAddress when premiumCollector is zero.
    /// @dev This test specifically covers line 83 in Loan.sol (the revert inside Loan constructor)
    function test_loan_constructor_zeroPremiumCollector_reverts() public {
        ConstructorParams memory p = _loadConstructorParams();
        ...
        new Loan(
            ...
            p.swapAdapterWrapper,
            p.zQuoter,
            address(0), // premiumCollector = address(0)
            p.preClosureFeeBps,
            p.gracePeriod,
            p.liquidationBuffer
        );
