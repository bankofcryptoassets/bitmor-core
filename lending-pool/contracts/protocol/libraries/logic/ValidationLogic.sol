...
library ValidationLogic {
     ...
     * @dev Invariant 1.6: bvBTC shares (the collateral asset in BLP) MUST NOT be borrowable.
     * The reserve configuration for bvBTC MUST have `borrowingEnabled = false`, which causes
     * this function to revert via the `VL_BORROWING_NOT_ENABLED` check. This ensures bvBTC
     * shares can only serve as collateral and are never lent out.
     * @dev Invariant 7.4: After this borrow executes, `variableDebtToken.totalSupply()` MUST be
     * <= `IERC20(usdc).balanceOf(aTokenAddress)`. The collateral and LTV checks in this function
     * ensure that borrows MUST NOT exceed the deposited liquidity available in the pool.
    ...
    // /**
    //  * @dev Not used in our BITMOR Protocol
     ...
     * @dev Invariant 4.10: Full liquidation MUST only proceed when `typeOfLiquidation == 1`.
     * If `typeOfLiquidation != 1` (i.e., collateral is sufficient AND payment is current),
     * MUST revert with CANNOT_FULL_LIQUIDATE. When this validation passes and the liquidation
     * executes, the caller MUST set loan status to Liquidated.
    ...
    function validateLiquidationCall(
        ...
        if (typeOfLiquidation != 1) {
            return (uint256(Errors.CollateralManagerErrors.CANNOT_FULL_LIQUIDATE), Errors.LPCM_CANNOT_FULL_LIQUIDATE);
        }
     ...
     * @dev Invariant 4.4: Micro-liquidation MUST only proceed when `typeOfLiquidation == 2`,
     * which requires that a payment was missed AND the grace period has expired (enforced by
     * `checkTypeOfLiquidation`). If `typeOfLiquidation != 2`, MUST revert with
    ...
    function validateMicroLiquidationCall(
        ...
        if (typeOfLiquidation != 2) {
            return (uint256(Errors.CollateralManagerErrors.CANNOT_MICRO_LIQUIDATE), Errors.LPCM_CANNOT_MICRO_LIQUIDATE);
        }
