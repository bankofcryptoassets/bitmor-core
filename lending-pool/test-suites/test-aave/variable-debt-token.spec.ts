...
makeSuite("Variable debt token tests", (testEnv: TestEnv) => {
    ...
    describe("AaveOracle: coverage", () => {
        ...
        it("getAssetPrice enters bvBTC vault conversion path", async () => {
            ...
            // bvBTC price = btcPrice * assetPerShare / 10^btcDecimals
            // With 1:1 mock vault, assetPerShare = oneShare, so bvBTC price = btcPrice
            const price = await aaveOracle.getAssetPrice(getContractAddress(btcVault));
    ...
    describe("DebtTokenBase: disabled ERC20 functions", () => {
        it("transfer reverts with TRANSFER_NOT_SUPPORTED", async () => {
            ...
            await expect(variableDebtContract.transfer(users[0].address, "1")).to.be.revertedWith(
        ...
        it("approve reverts with APPROVAL_NOT_SUPPORTED", async () => {
            ...
            await expect(variableDebtContract.approve(users[0].address, "1")).to.be.revertedWith(
